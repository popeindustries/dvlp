...
import { getAbsoluteProjectPath, getProjectPath, isEsmFile } from './file.ts';
...
import { filePathToUrlPathname } from './url.ts';
...
export function patchResponse(
  req: Req,
  res: Res,
  { footerScript, headerScript, resolveImport, send }: PatchResponseOptions,
) {
 ...
 */
function disableContentEncodingHeader(
  res: Res,
  headerKey: string,
  headerValue: string,
): string | undefined {
 ...
 */
function enableCrossOriginHeader(res: Res) {
...
function injectScripts(
  res: Res,
  scripts: { footer: string; header: string },
  html: string,
): string {
 ...
 */
function injectCSPMetaTag(res: Res, html: string, urls: Array<string>): string {
...
function rewriteCSSImports(
  ...
  filePath: string,
  css: string,
  resolveImport?: PatchResponseOptions['resolveImport'],
): string {
  ...
  try {
    ...
    if (imports.length > 0) {
      ...
      for (const { a: assertion, d: dynamic, e, s, se } of imports) {
        ...
        let after = '';
        let before = '';
        let assert: ImportAssertionType = undefined;
        ...
        if (isAssert) {
          ...
          if (match) {
            assert = match[1] as ImportAssertionType;
          }
        }
        ...
        if (resolveImport) {
          ...
          try {
            ...
            (err as Error & { hooked: boolean }).hooked = true;
            throw err;
          }
    ...
    if ((err as Error & { hooked: boolean }).hooked) {
      fatal(err);
    }
  }
 ...
 */
function proxySetHeader(
  res: Res,
  action: (key: string, value: string) => string | undefined,
) {
...
function parseCSPRules(cspString: string, urlsString: string) {
  const rules = cspString
    .split(';')
    .map((ruleString) => ruleString.trim())
    .reduce((rules: { [key: string]: string }, ruleString) => {
