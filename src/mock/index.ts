import { error, noisyInfo } from '../utils/log.ts';
import { fileURLToPath, pathToFileURL, URLSearchParams } from 'node:url';
import { getUrl, isEqualSearchParams } from '../utils/url.ts';
import {
  isInvalidFilePath,
  isJsFilePath,
  isJsonFilePath,
...
export class Mocks {
  ...
  addPushEvents(
    ...
    for (const event of events) {
      ...
      if (event.name) {
        const options: MockStreamEventData['options'] = {
          delay: 0,
          ...event.options,
          protocol,
        };

        const sequence: Array<MockStreamEventData> = [];
   ...
   */
  async load(filePaths: string | Array<string>): Promise<void> {
   ...
   * @private
   */
  async loadFilePath(filePath: string) {
  ...
  async loadFile(filePath: string) {
    try {
      let mocks: Array<MockResponseJSONSchema | MockPushEventJSONSchema>;
 ...
 * @private
 */
async function triggerEventSequence(
  stream: string | MockPushStream,
  eventSequence: Array<MockStreamEventData>,
  push: (stream: string | PushStream, event: PushEvent) => void,
): Promise<void> {
...
function isMock(json: object): boolean {
  return (
 ...
 */
function isValidMockFormat(mock: { [key: string]: any }): boolean {
  return (
    ('request' in mock &&
      'url' in mock.request &&
      'response' in mock &&
 ...
 */
function isMockRequest(req: any): req is MockRequest {
  return (
    req &&
    typeof req === 'object' &&
    req.url !== undefined &&
 ...
 */
function isMockResponseData(mock: any): mock is MockResponseData {
 ...
 */
function isMockStreamData(mock: any): mock is MockStreamData {
 ...
 */
function normaliseHeaderKeys(
  headers: { [key: string]: string },
  keys: Array<string>,
) {
  const normalisedHeaders: { [key: string]: string } = {};
