...
module.exports = class DvlpServer {
  ...
  createRequestHandler(originalRequestHandler) {
    return async function requestHandler(req, res) {
      ...
      const instance = /** @type { DvlpServer } */ this;
      let filePath = instance.urlToFilePath.get(req.url);

      res.url = req.url;

      if (
        handleMockResponse(req, res, instance.mocks) ||
        handlePushEvent(req, res, instance.mocks)
      ) {
        return;
      }
      ...
      if (type !== 'html' && (!filePath || !fs.existsSync(filePath))) {
        ...
        if (filePath) {
          instance.urlToFilePath.set(req.url, filePath);
          ...
          if (isModuleBundlerFilePath(filePath)) {
            await bundle(
              ...
              undefined,
              undefined,
              instance.rollupConfig
            );
          }
        } else {
          // File not found. Clear previously known path
          instance.urlToFilePath.delete(req.url);
        }
      }

      // Ignore unknow types
      if (type) {
        patchResponse(filePath, req, res, instance.patchResponseOptions);
      }
