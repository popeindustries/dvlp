...
module.exports = async function staticServer(
  ...
  const instance = await factory(cwd, directories, port, reloadServer, rollupConfig);
 ...
 */
async function factory(cwd, directories, port, reloadServer, rollupConfig) {
  const isReloading = reloadServer !== undefined;
  const reloadPort = reloadServer ? reloadServer.port : undefined;
  const server = await start(cwd, directories, port, isReloading, reloadPort, rollupConfig);
  let watcher;
 ...
 * @param {number} reloadPort
 * @param {object} [rollupConfig]
 * @returns {Promise<http.Server>}
 */
function start(cwd, directories, port, isReloading, reloadPort, rollupConfig) {
  return new Promise((resolve, reject) => {
    const server = http.createServer(async (req, res) => {
      ...
      patchRequest(req);
      patchResponse(req, res, isReloading, reloadPort);
