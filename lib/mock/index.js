  ...
  initRequestInterception() {
    this.uninterceptClientRequest = interceptClientRequest((url) => {
      if (this.hasMatch(url.href)) {
        url.searchParams.append('mock', encodeURIComponent(url.href));
...
function isMockFile(json) {
  return 'request' in json || 'response' in json || 'push' in json;
}
...
function isValidSchema(json) {
  return (
    ...
    ('push' in json && 'url' in json.push && 'events' in json.push)
  );
}
