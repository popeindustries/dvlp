...
module.exports = class Mock {
  ...
  addResponse(req, res, once = false, onMockCallback) {
    ...
    const [origin, pathRegex, searchParams] = getUrlMatchers(req, ignoreSearch);
  ...
  addPushEvents(stream, events) {
    ...
    const [origin, pathRegex, searchParams] = getUrlMatchers(
      stream,
      ignoreSearch
    );
    /** @type { MockStreamType } */
    ...
    /** @type { MockStreamEventsData } */
    const eventsData = {};

    // for (const event of events) {
    // Ignore events without a name
    // if (event.name) {
    //   eventsData[event.name] = { ...event };
    //   // Set options for ws
    //   if (!event.options) {
    //   }
    //   if (type === 'ws') {
   ...
   *
   * @param { string | Req | URL | MockRequest | MockPushStream | MockResponseData | MockStreamData } reqOrMockData
   * @returns { void }
   */
  ...
  /**
   * Retrieve response mock
   *
   * @param { string | URL | Req | MockRequest } req
 ...
 */
function getUrlMatchers(req, ignoreSearch) {
