...
module.exports = class Mock {
  ...
  constructor(filePaths) {
    /** @type { Map<string, MockResponseData | MockStreamData> } */
    this.cache = new Map();
  ...
  matchResponse(key, req, res) {
    ...
    if (typeof mock.response === 'function') {
      return mock.response(mock.url, req, res);
    }
    ...
    noisyInfo(
      `${stopwatch.stop(
        ...
        decodeURIComponent(mock.url.href)
      )}`
    );
  ...
  remove(keyOrObjectWithUrl) {
    const mockData = this.getMock(keyOrObjectWithUrl);

    if (!mockData) {
      return;
    }

    const cacheKey = getUrlCacheKey(mockData.url);
 ...
 *
 * @param { MockResponseData | MockStreamData } mock
