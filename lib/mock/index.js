...
const { getUrl, isWebSocketUrl } = require('../utils/url.js');
...
module.exports = class Mock {
  ...
  constructor(filePaths) {
    /** @type { Set<MockResponseData> } */
    this.responseCache = new Set();
    /** @type { Set<MockStreamData> } */
    ...
    //   /\$MOCKS/g,
    //   JSON.stringify(
    //     [...this.responseCache].map(([key, value]) => {
  ...
  addResponse(req, res, once = false, onMockCallback) {
    ...
    const [origin, pathRegex, searchParams] = getUrlSegmentsForMatching(
      req,
      ignoreSearch
    );
  ...
  addPushEvents(stream, events) {
    ...
    const type = isWebSocketUrl(new URL(origin)) ? 'ws' : 'es';
    ...
    for (const event of events) {
      ...
      if (event.name) {
        ...
        if (type === 'ws') {
          eventData.options.protocol = protocol;
        }

        eventsData[event.name] = eventData;
      }
    }
  ...
  matchResponse(href, req, res) {
    const mock = this.getMockResponseData(href);
    ...
    if (typeof mock.response === 'function') {
      return mock.response(getUrl(href), req, res);
    }
  ...
  hasMatch(reqOrMockData) {
    if (isMockResponseData(reqOrMockData)) {
      return this.responseCache.has(reqOrMockData);
    }
    if (isMockStreamData(reqOrMockData)) {
      return this.streamCache.has(reqOrMockData);
    }
    return (
      this.getMockResponseData(reqOrMockData) !== undefined ||
      this.getMockPushData(reqOrMockData) !== undefined
    );
  }
  ...
  remove(reqOrMockData) {
    if (isMockResponseData(reqOrMockData)) {
      this.responseCache.delete(reqOrMockData);
    } else if (isMockStreamData(reqOrMockData)) {
      this.streamCache.delete(reqOrMockData);
    } else {
      const mockResponseData = this.getMockResponseData(reqOrMockData);

      if (mockResponseData) {
        this.responseCache.delete(mockResponseData);
      } else {
        const mockStreamData = this.getMockPushData(reqOrMockData);

        if (mockStreamData) {
          this.streamCache.delete(mockStreamData);
        }
      }
    }
  ...
  /**
   * Retrieve mock response data
   *
   * @param { string | URL | { url: string } } req
   * @returns { MockResponseData | undefined }
   * @private
   */
  getMockResponseData(req) {
    const url = getUrl(req);

    for (const mock of this.responseCache) {
      if (
        url.origin !== mock.origin ||
        (!mock.ignoreSearch &&
  ...
  /**
   * Retrieve mock stream data
   *
   * @param { string | URL | { url: string } } req
   * @returns { MockStreamData | undefined }
   * @private
   */
  getMockPushData(req) {
    const url = getUrl(req);

    for (const mock of this.streamCache) {
      if (
        url.origin !== mock.origin ||
        (!mock.ignoreSearch &&
          !isEqualSearchParams(url.searchParams, mock.searchParams))
 ...
 * @returns { [string, RegExp, URLSearchParams] }
 */
function getUrlSegmentsForMatching(req, ignoreSearch) {
  ...
  return [origin, pathToRegexp(pathname), new URLSearchParams(search)];
}
