...
module.exports = class Mock {
   ...
   *
   * @param { string | Req | MockRequest | MockPushStream } keyOrMock
   * @returns { boolean }
   */
  hasMatch(keyOrMock) {
    return this.getMock(keyOrMock) !== undefined;
  }
   ...
   */
  remove(keyOrMock) {
    const { key, url } = this.getMock(keyOrMock);
   ...
   * Retrieve mock
   *
   * @param { string | Req | MockRequest | MockPushStream } req
   * @returns { object }
   * @private
   */
  getMock(req) {
