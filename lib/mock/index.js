...
module.exports = class Mock {
  ...
  addPushEvents(stream, events) {
    ...
    const protocol = stream.protocol || type;
   ...
   *
   * @param { string | import("http").IncomingMessage } req
  ...
  matchPushEvent(stream, name, push) {
    ...
    if (name !== 'connect' || (name === 'connect' && event !== undefined)) {
      ...
      sequence = Object.keys(mock.events).reduce((sequence, name) => {
        const event = mock.events[name];

        if (event.connect) {
          event.sequence === undefined
            ? sequence.push({ message: event.message, options: event.options })
   ...
   *
   * @param { string | import("http").IncomingMessage | MockPushStream } reqOrStream
   * @returns { void }
   */
  remove(reqOrStream) {
