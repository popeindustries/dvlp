 ...
 *
 * @param { ClientRequest | string } req
...
function find(
  ...
  if (!isRequestObject) {
    requestedFilePath = req.includes(process.cwd()) ? getProjectPath(req) : req;
  } else {
    requestedFilePath = new URL(req.url, 'http://localhost').pathname;
  }
  ...
  if (isModuleBundlerFilePath(requestedFilePath)) {
    ...
    for (let directory of directories) {
      console.log(directory, requestedFilePath);
 ...
 * @returns { string }
 */
function getAbsoluteProjectPath(projectFilePath) {
  return path.resolve(
    projectFilePath.charAt(0) === '/'
...
/**
 * Determine if 'filePath' is a filepath or package reference
 *
 * @param { string } filePath
 * @returns { boolean }
 */
function isFilePath(filePath) {
  return (
    filePath && (isAbsoluteFilePath(filePath) || isRelativeFilePath(filePath))
  );
}
