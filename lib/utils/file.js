...
const loadESModule = require('esm')({});
  ...
  return resolve(id, {
    ...
    packageFilter: (pkg) => {
      ...
      if (pkg.module !== undefined) {
        pkg.main = cleanFilepath(pkg.module);
      }
      if (pkg['jsnext:main'] !== undefined) {
        pkg.main = cleanFilepath(pkg['jsnext:main']);
      }
      if (pkg.main !== undefined) {
        pkg.main = cleanFilepath(pkg.main);
      }
      if (pkg.browser !== undefined) {
        if (typeof pkg.browser === 'string') {
          pkg.main = cleanFilepath(pkg.browser);
        } else {
          for (const f in pkg.browser) {
            pkg.browser[cleanFilepath(f)] = cleanFilepath(pkg.browser[f]);
          }
