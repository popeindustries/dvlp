...
function find(req, { directories = config.directories, type } = {}) {
  const requestedFilePath = isRequest(req)
    ? new URL(req.url, 'http://localhost').pathname
    : req;
  let filePath;

  if (!type) {
    type = isRequest(req) ? getTypeFromRequest(req) : getTypeFromPath(req);
  }
...
/**
 * Determine whether "req" is a request instance
 *
 * @param { unknown } req
 * @returns { req is import("http").IncomingMessage }
 */
