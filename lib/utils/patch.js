...
const { bundle, resolveModuleId } = require('./moduleBundler');
...
const { moduleCacheDirName } = require('../config');
...
function patchResponse(req, res, { directories, rollupConfig, scriptString } = {}) {
  if (isHtmlRequest(req)) {
    ...
    proxyBodyWrite(res, rewriteImports.bind(rewriteImports, directories, rollupConfig));
  }
}
 ...
 */
function rewriteImports(directories, rollupConfig, data) {
  ...
  while ((match = RE_IMPORT.exec(str))) {
    ...
    if (RE_BARE_IMPORT.test(id)) {
      ...
      if (resolvedId) {
        ...
        newId = `/${path.join(moduleCacheDirName, resolvedId)}`;
        warn(WARN_BARE_IMPORT, id);
      }
    } else if (!path.extname(id)) {
      const req = { url: id };
      const filepath = find(req, { directories, type: 'js' });
