...
const eventSourceClient = fs.readFileSync(
  path.resolve(__dirname, './eventSourceClient.js'),
  'utf8'
);
 ...
 */
function patchResponse(req, res, reload) {
...
function injectAtClosingBody(port, data) {
  ...
  data = data.replace(RE_BODY_TAG, `${eventSourceClient}\n</body>`);
