...
const { find, getAbsoluteProjectPath, getProjectPath } = require('./file.js');
const {
  isBareImport,
  isCssRequest,
  isHtmlRequest,
...
function rewriteImports(filePath, directories, rollupConfig, data) {
  ...
  while ((match = RE_IMPORT.exec(code))) {
    ...
    if (isBareImport(id) || parentIsNodeModule) {
      importPath = resolve(getAbsoluteProjectPath(filePath), id);
      ...
      if (importPath !== undefined) {
        ...
        if (process.env.NODE_PATH !== undefined) {
          ...
          importPath = find(id, {
            directories: nodePathDirectories,
            type: 'js'
          });
          warn(WARN_NODE_PATH);
        }
      ...
      importPath = find(getProjectPath(path.join(path.dirname(filePath), id)), {
        directories,
        type: 'js'
      });
    ...
    if (newId) {
      ...
      code = code.replace(context, `${pre}${newId}${post}`);
    }
  }
