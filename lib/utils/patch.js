 ...
 *  - {string} [scriptUrl]
 */
function patchResponse(req, res, { directories, rollupConfig, scriptString, scriptUrl } = {}) {
  if (isHtmlRequest(req)) {
    debug(`handling html response for "${req.url}"`);
    if (scriptUrl) {
      proxySetHeader(res, injectCSPHeader.bind(injectCSPHeader, scriptUrl, scriptString));
    }
    if (scriptString) {
