...
const bundleWorker = require('./bundle');
...
function doBundle(id, resolvedId, filepath) {
  return (cache[resolvedId] = new Promise(async (resolve, reject) => {
    ...
    bundler(id, resolvedId, filepath, LOCAL_CACHE_DIR, (err) => {
      if (err) {
...
/**
 * Start workers if WORKERIZE flag set, otherwise use bundler directly
 */
function getBundler() {
  if (!MAX_WORKERS) {
    return bundleWorker;
  ...
  if (!workers) {
    workers = workerFarm({ maxConcurrentWorkers: MAX_WORKERS }, require.resolve('./bundle.js'));
