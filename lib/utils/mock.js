 ...
 */
function load(filepaths, newHost) {
  ...
  for (let filepath of filepaths) {
    ...
    try {
      ...
      if (stat.isDirectory()) {
        loadDirectory(filepath, newHost);
      } else {
        loadFile(filepath, newHost);
      }
    } catch (err) {
 ...
 * @param {string} [newHost]
 */
function loadDirectory(dirpath, newHost) {
  fs.readdirSync(dirpath).forEach((filepath) => {
    load(path.join(dirpath, filepath), newHost);
  });
}
 ...
 * @param {string} [newHost]
 */
function loadFile(filepath, newHost) {
  ...
  try {
    ...
    for (const mock of mocks) {
      ...
      if (newHost && RE_REMOTE_URL.test(request.url)) {
        const url = new URL(request.url);
        const overrideUrl = new URL(newHost);
