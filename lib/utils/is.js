 ...
 * @returns { boolean }
 */
function isProjectFilePath(filePath, includeNodeModules = false) {
  if (!path.isAbsolute(filePath)) {
    filePath = path.resolve(filePath);
  }

  const included = includeNodeModules ? true : !isNodeModuleFilePath(filePath);

  return filePath.includes(process.cwd()) && included;
}
