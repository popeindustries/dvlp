...
module.exports = async function server(
  ...
  if (Array.isArray(filepath) || fs.statSync(path.resolve(filepath)).isDirectory()) {
    server = await staticServer(filepath, { port, reloadServer: rServer, rollupOptions: config });
  } else {
    server = await appServer(filepath, { port, reloadServer: rServer, rollupOptions: config });
  }
