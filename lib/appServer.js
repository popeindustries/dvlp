...
class AppServer {
  ...
  createRequestHandler(originalRequestHandler) {
    return async function requestHandler(req, res) {
      ...
      res.once('finish', () => {
        const duration = stopwatch.stop(req.url, true, true);

        if (mocked) {
          info(`${duration} intercepted request for mocked ${chalk.green(mocked)}`);
        } else {
          info(`${duration} handled request for ${chalk.green(req.url)}`);
        }
      });
      ...
      if (!filepath || !fs.existsSync(filepath)) {
        try {
          ...
          debug(`matched "${req.url}" to "${filepath}"`);
      ...
      // Send file if found, otherwise let application handle it
      if (filepath) {
        this.watcher.add(filepath);
        if (this.transpiler) {
