...
const STOP_TIMEOUT_DURATION = 500;
...
async function factory(main, reloadServer, rollupConfig, fn) {
  ...
  let [server, appModules] = await start(main, isReloading, reloadPort, rollupConfig);
  const watcher = await watch(reloadServer ? process.cwd() : appModules, async (filepath) => {
    ...
    try {
      ...
      const [newServer, newAppModules] = await start(main, isReloading, reloadPort, rollupConfig);
 ...
 */
function start(main, isReloading, reloadPort, rollupConfig) {
