...
class AppServer {
  ...
  /**
   * Create request listener wrapper for 'originalRequestListener'
   * @param {(http.ClientRequest, http.ServerResponse) => void} originalRequestListener
   * @returns {(http.ClientRequest, http.ServerResponse) => void}
   */
  createRequestListener(originalRequestListener) {
    return async function requestListener(req, res) {
      ...
      if (!filepath) {
        try {
          ...
          filepath = await find(req, this.directories, this.rollupConfig);
      ...
      patchRequest(req);
      patchResponse(req, res, this.reloader && this.reloader.client);
      ...
      http.createServer = new Proxy(http.createServer, {
        apply(target, ctx, args) {
          ...
          args[args.length - 1] = instance
            .createRequestListener(args[args.length - 1])
            .bind(instance);
