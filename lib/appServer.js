...
const { findFile, injectReloadScript, isHtmlRequest, isJsRequest, log, watch } = require('./utils');
...
/**
 * Start app server.
 * Patches createServer to grab instance and register for events,
...
function start(main, isReloading) {
  return new Promise((resolve, reject) => {
    http.createServer = (...args) => {
      ...
      const server = originalCreateServer(hasOptions ? args[0] : {}, (req, res) => {
        if (isJsRequest(req)) {
