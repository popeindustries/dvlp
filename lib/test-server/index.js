...
class TestServer {
      ...
      if (this._enableWebSocket) {
        this._server.on('upgrade', (req, socket, body) => {
          if (WebSocket.isWebSocket(req)) {
            this._initClient('ws', req, socket, body);
          }
        });
      }
  ...
  _initClient(type, ...args) {
    ...
    debug(`added ${type} connection`, this._clients.size);

    client.on('close', () => {
      this._clients.delete(client);
      debug(`removed ${type} connection`, this._clients.size);
    });
    if (isWebSocket) {
